...
and patt_cst =
  cons * (string * type_cst) list
and copatt_cst =
  cons * string * type_cst * (string * type_cst) list

(* Grammaire *)
(*   cmd = (jump {n} t S) | (call {p} V e) *)
(*   t = V | (force !a {p} cmd) *)
(*   e = S | (force x {n} cmd) *)
(*   V = x | :k | (let !a {n} cmd) | (box K !alpha {a} cmd) | (:k V...) | (match (:k !a {a} x {a}...) cmd ...) *)
(*   S = !a | (let x {p} cmd) | (unbox K S) | (:k !a V...) | (match (:k x {a}...) cmd...) *)
(*   a = p | n | Tvar *)
(*   p = {kt+ a...} | Tvar+ *)
(*   n = {kt- a...} | Tvar- *)
